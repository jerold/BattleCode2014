        ...
        while (!rc.getLocation().equals(target) || rc.getLocation().isAdjacentTo(target))
        {
            ...
            try
            {
                ...
                else if (rc.isActive())
                {
                    ...
                    {
                        alliedBots = FightMicro2.AllAlliedBotsInfo(rc);
                    }
                    if (AllEnemyBots.length == 0 && nearByEnemies.length > 0)
                    {
                        FightMicro2.FindAndRecordAllEnemies(rc, nearByEnemies, AllEnemyBots, AllEnemyNoiseTowers);
                        AllEnemyBots = FightMicro2.AllEnemyBots(rc);
                    }
                                ...
                                try
                                {
                                    ...
                                    if (FightMicro2.fightMode(rc, target))
                                    {
                                        ...
                                        if (rc.isActive())
                                        {
                                            ...
                                            if (nearByEnemies33.length > 0)
                                            {
                                                AllEnemyBots33 = FightMicro2.AllEnemyBots(rc);
                                                AllEnemyNoiseTowers33 = null;

                                                FightMicro2.FindAndRecordAllEnemies(rc, nearByEnemies33, AllEnemyBots33, AllEnemyNoiseTowers33);
    ...
    public static void fire(RobotController rc, Robot[] enemies, MapLocation[] allyBots)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                enemies = FightMicro2.findSoldiersAtDistance(rc, enemies2, radius);
                Robot target = null;
                ...
                if(target != null)
                {
                    if (rc.canSenseObject(target))
                    {
                    	if (rc.canAttackSquare(rc.senseRobotInfo(target).location))
                    	{
                            ...
                            {
                                int[] enemyRobots = FightMicro2.AllEnemyBots(rc);
                                FightMicro2.recordEnemyBotKilled(rc, enemyRobots, target);
                            }
                    	}
                    }
